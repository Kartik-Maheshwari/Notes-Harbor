...
const SingleCard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/v1/uploads/${asset_id}`);
      ...
      <div className="w-full max-w-4xl bg-white rounded-lg shadow-lg overflow-hidden relative transition-transform duration-300 transform hover:scale-105 border-2 border-transparent">
        ...
        <div className="p-6 space-y-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              <button type="button" onClick={toggleLike}>
                <AiOutlineHeart className={`text-3xl transition-transform duration-300 ${liked ? 'text-red-500 animate-bounce' : 'text-gray-500'}`} />
              </button>
              <span className="text-lg font-semibold">{liked ? "1 Like" : "0 Likes"}</span>
            </div>
            <div className="text-sm text-gray-600 flex space-x-6">
              <p>Uploaded by: <span className="font-bold">{cardData.uploader}</span></p>
              <p>Semester: {cardData.semester}</p>
            </div>
          </div>

          <p className="text-base text-gray-700">
            {showMore ? cardData.description : `${cardData.description?.substring(0, 150)}...`}
          ...
          <div className="flex justify-start space-x-4 mt-4">
            <button type="button" className="flex items-center space-x-2" onClick={toggleCommentBox}>
              <AiOutlineComment className="text-3xl animate-pulse text-gray-500" />
              <span className="text-lg">Comments ({comments.length})</span>
            </button>
            <div className="flex items-center space-x-4">
            <button type="button" className="flex items-center space-x-2" onClick={toggleShareOptions}>
...
{showShareOptions && (
  <div className="flex flex-col space-y-2 mt-2">
    <button onClick={() => handleShareOptionClick('copy')} className="text-blue-500">
      Copy Link
    </button>
    
    <button onClick={() => handleShareOptionClick('native')} className="text-blue-500">
      Native Share
    </button>
  </div>
)}
